...
class ColumnSelectorApp:
    def __init__(self, root):
        ...
        self.tree.grid(row=3, column=2, columnspan=10, rowspan=2, pady=10)
    ...
    def toggle_column(self, index):
        ...
        self.label.configure(text=f"Selected columns: {self.selected_columns}")
...
run_button2 = ctk.CTkButton(tabview.tab("Michelis-Menten Kinetics"), text="Graph Averages M-M", command=graph_mm_averages)
run_button2.grid(row=1, column=1, pady=10, padx=15)

run_button2a = ctk.CTkButton(tabview.tab("Michelis-Menten Kinetics"), text="Graph Best Fit M-M", command=graph_mm_best_fit)
...
path_label.grid(row=2, column=2, columnspan=3, pady=10)

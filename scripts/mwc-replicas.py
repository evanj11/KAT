...
from mwc_kinetic_analysis import get_parser, Import_Kinetic_Data, MWC_Kinetic_Solver, get_inputs, graph_kinetic_data
...
def bayesian_bootstrap(s, v, best_params, n_iter=1000):
    ...
    val = MWC_Kinetic_Solver(best_params[0], best_params[1], best_params[2], best_params[3], best_params[4], best_params[5])
    for _ in range(n_iter):
        weights = rng.dirichlet(np.ones(n)*4)
        def weighted_rss(params):
            v_pred = val.mwc_model(s)
...
def main():
    ...
    print("<span style='color:blue;'>Running Monod-Wyman-Changeux Replica Kinetic Analysis\n</span>")
    ...
    for i in range(len(pathds)):
        ...
        bounds = [(0, 100000), (0, 100000), (1e-6, 100000), (1e-6, 10000), (1e-3, 10000), (0.5, 20)]
        ...
        for _ in range(100):
            guess = np.random.uniform([0, 0, 0, 0, 0, 0], [1000, 1000, 1000, 1000, 500, 14])
        ...
        if len(substrate) < 30:
            ...
            boot_params = bayesian_bootstrap(substrate, vvalues, cv_mean_params, n_iter=1000)
        ...
        val = MWC_Kinetic_Solver(kinetic_parameters[0], kinetic_parameters[1], kinetic_parameters[2], kinetic_parameters[3], kinetic_parameters[4], kinetic_parameters[5])
        vval_calc = val.mwc_model(substrate)
        ...
        with open(os.path.join(work_dir, f'{file_name[0]}_{i}.txt'), 'w') as file:
            ...
            file.write("\n")
            file.write('Monod-Wyman-Changeux')
            file.write("\n")
            file.write('Best-Fit Parameters:\n') 
            file.write('V_T = ' + str(kinetic_parameters[0]))
            file.write("\n")
            file.write('V_R = ' + str(kinetic_parameters[1]))
            file.write("\n")
            file.write('K_T = ' + str(kinetic_parameters[2]))
            file.write("\n")
            file.write('K_R = ' + str(kinetic_parameters[3]))
            file.write("\n")
            file.write('L0 = ' + str(kinetic_parameters[4]))
            file.write("\n")
            file.write('n = ' + str(kinetic_parameters[5]))
            file.write("\n")
            file.write('Minimized residual sum of squares: ' + str(refined.fun))
            file.write("\n\n")
            file.write('Cross-Validation (KFold, k=5) Mean Parameters:\n')
            file.write('V_T = ' + str(cv_mean_params[0]))
            file.write("\n")
            file.write('V_R = ' + str(cv_mean_params[1]))
            file.write("\n")
            file.write('K_T = ' + str(cv_mean_params[2]))
            file.write("\n")
            file.write('K_R = ' + str(cv_mean_params[3]))
            file.write("\n")
            file.write('L0 = ' + str(cv_mean_params[4]))
            file.write("\n")
            file.write('n = ' + str(cv_mean_params[5]))
            file.write("\n")
        ...
        with open(os.path.join(work_dir, f'complex_data_{i}.txt'), 'w') as file:
            ...
            file.write("\n")
            file.write(str(kinetic_parameters[4]))
            file.write("\n")
            file.write(str(kinetic_parameters[5]))
            file.write("\n")
            if len(substrate) < 30:
                if 'False' in within_ci:
                    file.write('\n')
    ...
    with open(os.path.join(work_dir, f'{file_name[0]}.txt'), 'w') as file:
        ...
        file.write('Monod-Wyman-Changeux')
        file.write("\n")
        vt_avg = '%.3f'%(kinetic_parameters_avg[0])
        vt_std = '%.3f'%(kinetic_parameters_std[0])
        file.write('V_T = ' + str(vt_avg))
        file.write(" \u00B1 ")
        file.write(str(vt_std))
        file.write("\n")
        vr_avg = '%.3f'%(kinetic_parameters_avg[1])
        vr_std = '%.3f'%(kinetic_parameters_std[1])
        file.write('V_R = ' + str(vr_avg))
        file.write(" \u00B1 ")
        file.write(str(vr_std))
        file.write("\n")
        kt_avg = '%.3f'%(kinetic_parameters_avg[2])
        kt_std = '%.3f'%(kinetic_parameters_std[2])
        file.write('K_T = ' + str(kt_avg))
        file.write(" \u00B1 ")
        file.write(str(kt_std))
        file.write("\n")
        kr_avg = '%.3f'%(kinetic_parameters_avg[3])
        kr_std = '%.3f'%(kinetic_parameters_std[3])
        file.write('K_R = ' + str(kr_avg))
        file.write(" \u00B1 ")
        file.write(str(kr_std))
        file.write("\n")
        l0_avg = '%.3f'%(kinetic_parameters_avg[4])
        l0_std = '%.3f'%(kinetic_parameters_std[4])
        file.write('L0 = ' + str(l0_avg))
        file.write(" \u00B1 ")
        file.write(str(l0_std))
        file.write("\n")
        n_avg = '%.3f'%(kinetic_parameters_avg[5])
        n_std = '%.3f'%(kinetic_parameters_std[5])
        file.write('N = ' + str(n_avg))
        file.write(" \u00B1 ")
        file.write(str(n_std))
        file.write("\n")
    ...
    with open(os.path.join(work_dir, f'{file_name[0]}_complex_data.txt'), 'w') as file:
        ...
        vt_avg = '%.3f'%(kinetic_parameters_avg[0])
        vt_std = '%.3f'%(kinetic_parameters_std[0])
        file.write(str(vt_avg))
        file.write(" \u00B1 ")
        file.write(str(vt_std))
        file.write("\n")
        vr_avg = '%.3f'%(kinetic_parameters_avg[1])
        vr_std = '%.3f'%(kinetic_parameters_std[1])
        file.write(str(vr_avg))
        file.write(" \u00B1 ")
        file.write(str(vr_std))
        file.write("\n")
        kt_avg = '%.3f'%(kinetic_parameters_avg[2])
        kt_std = '%.3f'%(kinetic_parameters_std[2])
        file.write(str(kt_avg))
        file.write(" \u00B1 ")
        file.write(str(kt_std))
        file.write("\n")
        kr_avg = '%.3f'%(kinetic_parameters_avg[3])
        kr_std = '%.3f'%(kinetic_parameters_std[3])
        file.write(str(kr_avg))
        file.write(" \u00B1 ")
        file.write(str(kr_std))
        file.write("\n")
        l0_avg = '%.3f'%(kinetic_parameters_avg[4])
        l0_std = '%.3f'%(kinetic_parameters_std[4])
        file.write(str(l0_avg))
        file.write(" \u00B1 ")
        file.write(str(l0_std))
        file.write("\n")
        n_avg = '%.3f'%(kinetic_parameters_avg[5])
        n_std = '%.3f'%(kinetic_parameters_std[5])
        file.write(str(n_avg))
        file.write(" \u00B1 ")
        file.write(str(n_std))
        file.write("\n")
        if len(substrate) < 30:
    ...
    if 'mutant' in mut:
        with open(os.path.join(work_dir, f'{file_name[0]}_mutant_data.txt'), 'w') as file:
            for mut in range(len(kinetic_parameters_rep)):
                ...
                file.write(",")
                file.write(str("%.3f"%(kinetic_parameters_rep[mut][4])))
                file.write(",")
                file.write(str("%.3f"%(kinetic_parameters_rep[mut][5])))
                file.write("\n")
